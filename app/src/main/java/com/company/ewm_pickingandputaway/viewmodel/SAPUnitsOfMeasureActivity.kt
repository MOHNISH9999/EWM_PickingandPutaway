package com.company.ewm_putaway.mdui.sapunitsofmeasure
...
import com.company.ewm_putaway.R
import com.company.ewm_putaway.databinding.ActivityEntityitemBinding
import com.google.android.material.dialog.MaterialAlertDialogBuilder
import com.sap.cloud.mobile.flowv2.core.DialogHelper
import com.company.ewm_putaway.mdui.BundleKeys
import com.company.ewm_putaway.mdui.InterfacedFragment
import com.company.ewm_putaway.mdui.EntitySetListActivity
import com.company.ewm_putaway.mdui.UIConstants
import com.company.ewm_putaway.viewmodel.sapunitofmeasure.SAPUnitOfMeasureViewModel
 ...
 * - [SAPUnitsOfMeasureCreateFragment]: to create or edit a(n) SAPUnitOfMeasure,
 * - [SAPUnitsOfMeasureDetailFragment]: to display the details of a(n) SAPUnitOfMeasure,
 * - [SAPUnitsOfMeasureListFragment]: to list SAPUnitsOfMeasure.
 *
 ...
 * what to do.
 */
class SAPUnitsOfMeasureActivity : AppCompatActivity(), InterfacedFragment.InterfacedFragmentListener<SAPUnitOfMeasure> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, SAPUnitsOfMeasureListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit()
        } else {
    ...
    override fun onFragmentStateChange(evt: Int, entity: SAPUnitOfMeasure?) {
        when (evt) {
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link SAPUnitsOfMeasureCreateFragment}.
     */
    ...
    private fun onItemClicked(entity: SAPUnitOfMeasure?) {
        if (!isMasterDetailView) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, SAPUnitsOfMeasureDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if (detail == null) {
                if (entity != null)
                    supportFragmentManager.beginTransaction()
                        .replace(R.id.detailFrame, SAPUnitsOfMeasureDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                        .commit()
            } else {
                if( entity == null) {
    ...
    private fun onCreateNewItem() {
        ...
        val fragment = SAPUnitsOfMeasureCreateFragment()
    ...
    @Suppress("UNUSED_PARAMETER")
    private fun onEditItem(entity: SAPUnitOfMeasure?) {
        val updateFragment = SAPUnitsOfMeasureCreateFragment()
        val arguments = Bundle()
    ...
    internal class ConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            builder.setPositiveButton(R.string.before_navigation_dialog_positive_button) { _, _ ->
                (fragmentActivity as SAPUnitsOfMeasureActivity).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null)
            }
    ...
    internal class DeleteConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            val fragmentActivity = requireActivity() as AppCompatActivity
            val builder = MaterialAlertDialogBuilder(fragmentActivity, R.style.AlertDialogStyle)
            val viewModel = ViewModelProvider(fragmentActivity)[SAPUnitOfMeasureViewModel::class.java]
