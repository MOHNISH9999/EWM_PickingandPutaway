...
import com.company.ewm_pickingandputaway.archcomp.SingleLiveEvent
import com.company.ewm_pickingandputaway.repository.OperationResult.Operation

import com.sap.cloud.android.odata.crvwm_physicalinventory_srv_entities.CRVWM_PHYSICALINVENTORY_SRV_Entities
 ...
 * @param cRVWM_PHYSICALINVENTORY_SRV_Entities OData service
...
class Repository<T : EntityValue>(
        private val cRVWM_PHYSICALINVENTORY_SRV_Entities: CRVWM_PHYSICALINVENTORY_SRV_Entities,
    ...
    fun read() {
        ...
        cRVWM_PHYSICALINVENTORY_SRV_Entities.executeQueryAsync(dataQuery,
            { result ->
    ...
    fun read(parent: EntityValue, navigationPropertyName: String) {
        ...
        cRVWM_PHYSICALINVENTORY_SRV_Entities.loadPropertyAsync(navigationProperty, parent, dataQuery,
            {
    ...
    fun create(newEntity: T, media: StreamBase) {
        if (newEntity.entityType.isMedia) {
            cRVWM_PHYSICALINVENTORY_SRV_Entities.createMediaAsync(newEntity, media,
                {
    ...
    fun create(newEntity: T) {
        ...
        cRVWM_PHYSICALINVENTORY_SRV_Entities.createEntityAsync(newEntity,
            {
                insertToCache(newEntity, entities)
                if (relatedEntities.size > 0) {
                    insertToCache(newEntity, relatedEntities)
    ...
    fun update(updateEntity: T) {
        cRVWM_PHYSICALINVENTORY_SRV_Entities.updateEntityAsync(updateEntity,
            {
    ...
    fun delete(deleteEntities: MutableList<T>) {
        ...
        cRVWM_PHYSICALINVENTORY_SRV_Entities.applyChangesAsync(deleteChangeSet,
            {
    ...
    fun downloadMedia(entity:T, successHandler:Action1<ByteArray>,
        failureHandler:Action1<RuntimeException>) {
            cRVWM_PHYSICALINVENTORY_SRV_Entities.downloadMediaAsync(entity,
