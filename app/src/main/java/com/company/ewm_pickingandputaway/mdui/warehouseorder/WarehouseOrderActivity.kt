 ...
 * - [WarehouseOrderCreateFragment]: to create or edit a(n) WarehouseOrderType,
 * - [WarehouseOrderDetailFragment]: to display the details of a(n) WarehouseOrderType,
 * - [WarehouseOrderListFragment]: to list WarehouseOrder.
 *
 ...
 * what to do.
 */
class WarehouseOrderActivity : AppCompatActivity(), InterfacedFragment.InterfacedFragmentListener<WarehouseOrderType> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, WarehouseOrderListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit()
        } else {
    ...
    override fun onFragmentStateChange(evt: Int, entity: WarehouseOrderType?) {
        when (evt) {
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link WarehouseOrderCreateFragment}.
     */
    ...
    private fun onItemClicked(entity: WarehouseOrderType?) {
        if (!isMasterDetailView) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, WarehouseOrderDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if (detail == null) {
                if (entity != null)
                    supportFragmentManager.beginTransaction()
                        .replace(R.id.detailFrame, WarehouseOrderDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                        .commit()
            } else {
                if( entity == null) {
    ...
    private fun onCreateNewItem() {
        ...
        val fragment = WarehouseOrderCreateFragment()
    ...
    @Suppress("UNUSED_PARAMETER")
    private fun onEditItem(entity: WarehouseOrderType?) {
        val updateFragment = WarehouseOrderCreateFragment()
        val arguments = Bundle()
    ...
    internal class ConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            builder.setPositiveButton(R.string.before_navigation_dialog_positive_button) { _, _ ->
                (fragmentActivity as WarehouseOrderActivity).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null)
            }
    ...
    internal class DeleteConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            val fragmentActivity = requireActivity() as AppCompatActivity
            val builder = MaterialAlertDialogBuilder(fragmentActivity, R.style.AlertDialogStyle)
            val viewModel = ViewModelProvider(fragmentActivity)[WarehouseOrderTypeViewModel::class.java]
