 ...
 * - [SAPCurrenciesCreateFragment]: to create or edit a(n) SAPCurrency,
 * - [SAPCurrenciesDetailFragment]: to display the details of a(n) SAPCurrency,
 * - [SAPCurrenciesListFragment]: to list SAPCurrencies.
 *
 ...
 * what to do.
 */
class SAPCurrenciesActivity : AppCompatActivity(), InterfacedFragment.InterfacedFragmentListener<SAPCurrency> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, SAPCurrenciesListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit()
        } else {
    ...
    override fun onFragmentStateChange(evt: Int, entity: SAPCurrency?) {
        when (evt) {
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link SAPCurrenciesCreateFragment}.
     */
    ...
    private fun onItemClicked(entity: SAPCurrency?) {
        if (!isMasterDetailView) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, SAPCurrenciesDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if (detail == null) {
                if (entity != null)
                    supportFragmentManager.beginTransaction()
                        .replace(R.id.detailFrame, SAPCurrenciesDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                        .commit()
            } else {
                if( entity == null) {
    ...
    private fun onCreateNewItem() {
        ...
        val fragment = SAPCurrenciesCreateFragment()
    ...
    @Suppress("UNUSED_PARAMETER")
    private fun onEditItem(entity: SAPCurrency?) {
        val updateFragment = SAPCurrenciesCreateFragment()
        val arguments = Bundle()
    ...
    internal class ConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            builder.setPositiveButton(R.string.before_navigation_dialog_positive_button) { _, _ ->
                (fragmentActivity as SAPCurrenciesActivity).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null)
            }
    ...
    internal class DeleteConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            val fragmentActivity = requireActivity() as AppCompatActivity
            val builder = MaterialAlertDialogBuilder(fragmentActivity, R.style.AlertDialogStyle)
            val viewModel = ViewModelProvider(fragmentActivity)[SAPCurrencyViewModel::class.java]
