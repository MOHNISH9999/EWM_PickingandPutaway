 ...
 * - [WarehouseTaskCreateFragment]: to create or edit a(n) WarehouseTaskType,
 * - [WarehouseTaskDetailFragment]: to display the details of a(n) WarehouseTaskType,
 * - [WarehouseTaskListFragment]: to list WarehouseTask.
 *
 ...
 * what to do.
 */
class WarehouseTaskActivity : AppCompatActivity(), InterfacedFragment.InterfacedFragmentListener<WarehouseTaskType> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, WarehouseTaskListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit()
        } else {
    ...
    override fun onFragmentStateChange(evt: Int, entity: WarehouseTaskType?) {
        when (evt) {
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link WarehouseTaskCreateFragment}.
     */
    ...
    private fun onItemClicked(entity: WarehouseTaskType?) {
        if (!isMasterDetailView) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, WarehouseTaskDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if (detail == null) {
                if (entity != null)
                    supportFragmentManager.beginTransaction()
                        .replace(R.id.detailFrame, WarehouseTaskDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                        .commit()
            } else {
                if( entity == null) {
    ...
    private fun onCreateNewItem() {
        ...
        val fragment = WarehouseTaskCreateFragment()
    ...
    @Suppress("UNUSED_PARAMETER")
    private fun onEditItem(entity: WarehouseTaskType?) {
        val updateFragment = WarehouseTaskCreateFragment()
        val arguments = Bundle()
    ...
    internal class ConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            builder.setPositiveButton(R.string.before_navigation_dialog_positive_button) { _, _ ->
                (fragmentActivity as WarehouseTaskActivity).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null)
            }
    ...
    internal class DeleteConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            val fragmentActivity = requireActivity() as AppCompatActivity
            val builder = MaterialAlertDialogBuilder(fragmentActivity, R.style.AlertDialogStyle)
            val viewModel = ViewModelProvider(fragmentActivity)[WarehouseTaskTypeViewModel::class.java]
