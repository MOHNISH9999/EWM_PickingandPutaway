 ...
 * - [PhysicalInventoryCountSetCreateFragment]: to create or edit a(n) PhysicalInventoryCount,
 * - [PhysicalInventoryCountSetDetailFragment]: to display the details of a(n) PhysicalInventoryCount,
 * - [PhysicalInventoryCountSetListFragment]: to list PhysicalInventoryCountSet.
 *
 ...
 * what to do.
 */
class PhysicalInventoryCountSetActivity : AppCompatActivity(), InterfacedFragment.InterfacedFragmentListener<PhysicalInventoryCount> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, PhysicalInventoryCountSetListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit()
        } else {
    ...
    override fun onFragmentStateChange(evt: Int, entity: PhysicalInventoryCount?) {
        when (evt) {
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link PhysicalInventoryCountSetCreateFragment}.
     */
    ...
    private fun onItemClicked(entity: PhysicalInventoryCount?) {
        if (!isMasterDetailView) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, PhysicalInventoryCountSetDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if (detail == null) {
                if (entity != null)
                    supportFragmentManager.beginTransaction()
                        .replace(R.id.detailFrame, PhysicalInventoryCountSetDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                        .commit()
            } else {
                if( entity == null) {
    ...
    private fun onCreateNewItem() {
        ...
        val fragment = PhysicalInventoryCountSetCreateFragment()
    ...
    @Suppress("UNUSED_PARAMETER")
    private fun onEditItem(entity: PhysicalInventoryCount?) {
        val updateFragment = PhysicalInventoryCountSetCreateFragment()
        val arguments = Bundle()
    ...
    internal class ConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            builder.setPositiveButton(R.string.before_navigation_dialog_positive_button) { _, _ ->
                (fragmentActivity as PhysicalInventoryCountSetActivity).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null)
            }
    ...
    internal class DeleteConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            val fragmentActivity = requireActivity() as AppCompatActivity
            val builder = MaterialAlertDialogBuilder(fragmentActivity, R.style.AlertDialogStyle)
            val viewModel = ViewModelProvider(fragmentActivity)[PhysicalInventoryCountViewModel::class.java]
